...
def run_pythia():
    ...
    for msa_name in os.path.listdir(msa_dir):
        msa_path = os.path.join(msa_dir, msa_name)
        name = msa_name.split(".")[0]
        prefix = os.path.join(output_dir, name)
        pythia.run_with_padding(msa_path, prefix)
        results[name + ".rooted.tree"] = get_difficulty(prefix)

    pd.DataFrame(results, columns=['name', 'difficulty']).to_csv('data/bio/difficulties.csv', index=False)
    ...
    if not os.path.isdir(output_dir):
        os.makedirs(output_dir)
    for msa_subdir in os.path.listdir(msa_dir):
        msa_path = os.path.join(msa_dir, msa_subdir, "bin.phy")
        ds_id = msa_subdir.split("_")[0]
        prefix = os.path.join(output_dir, ds_id)
        pythia.run_with_padding(msa_path, prefix)
        results[ds_id + ".rooted.tree"] = get_difficulty(prefix)

    pd.DataFrame(results, columns=['name', 'difficulty']).to_csv('data/lang/difficulties.csv', index=False)
...
def correlations():
    for type in ["lang", "bio"]:
        ...
        result = []
        for metric_name in metrics.relative_metric:
            part_r = [metric_name]
